...
### 3. Session Pinning e Advisory Locks
...
- **Advisory Locks:** Usa locks nativos do PostgreSQL para serializar queries na mesma conexão

**Problema de Concorrência:**
- Múltiplas requisições simultâneas com mesmo Test-ID compartilham a mesma conexão/transação
- Queries executadas simultaneamente podem causar problemas (race conditions, deadlocks)
- Necessário serializar execução de queries na mesma transação

**Solução: Advisory Locks do PostgreSQL**
- Antes de executar query na conexão compartilhada, adquire advisory lock baseado no Test-ID
- Advisory lock garante que apenas uma query execute por vez na mesma transação
- Após execução, libera o advisory lock
- Outras requisições aguardam o lock ser liberado

**Implementação:**
```go
// Hash do Test-ID para usar como chave do advisory lock
...
// Adquire advisory lock antes de executar query
func (p *PGTest) acquireAdvisoryLock(session *TestSession, query string) error {
    lockKey := p.getAdvisoryLockKey(session.TestID)
    
    // pg_advisory_lock bloqueia até conseguir o lock
    _, err := session.Conn.Exec(context.Background(), 
        "SELECT pg_advisory_lock($1)", lockKey)
    return err
}

// Libera advisory lock após execução
func (p *PGTest) releaseAdvisoryLock(session *TestSession) error {
    lockKey := p.getAdvisoryLockKey(session.TestID)
    
    // pg_advisory_unlock libera o lock
    _, err := session.Conn.Exec(context.Background(), 
        "SELECT pg_advisory_unlock($1)", lockKey)
    return err
}

// Executa query com advisory lock
func (p *PGTest) executeWithLock(session *TestSession, query string) error {
    // Adquire lock
    if err := p.acquireAdvisoryLock(session, query); err != nil {
        return err
    }
    defer p.releaseAdvisoryLock(session) // Sempre libera
    
    // Executa query na transação compartilhada
    _, err := session.Conn.Exec(context.Background(), query)
    return err
}
```

**Vantagens dos Advisory Locks:**
- Nativo do PostgreSQL - não requer configuração adicional
- Funciona dentro da mesma transação
- Automático: locks são liberados quando a conexão fecha ou transação termina
- Eficiente: não bloqueia outras conexões, apenas queries na mesma conexão
- Serializa execução de queries na mesma transação compartilhada

**Diferença entre os dois tipos de locks:**
- **Locks em memória (sync.RWMutex):** Protegem acesso ao pool de sessões (evita race conditions ao criar/buscar sessões)
- **Advisory Locks (PostgreSQL):** Serializam execução de queries na mesma conexão/transação compartilhada (evita problemas quando múltiplas requisições executam queries simultaneamente)
...
type PGTest struct {
    ...
    mu             sync.RWMutex  // Lock para acesso thread-safe ao pool de sessões
}
```
...
3. **PGTest obtém/reusa sessão (com lock para thread-safety):**
   ```go
   session := pgtest.GetOrCreateSession(testID)
   // Usa lock (mu.Lock()) para garantir acesso thread-safe ao pool
   // Se não existe: cria conexão ao PostgreSQL real (SEM passar test_id)
   // Se existe: reutiliza conexão existente
   // Lock é liberado após obter/criar sessão
   ```
...
6. **Proxy de queries com Advisory Lock:**
   - Cliente envia Query
   - PGTest intercepta BEGIN/COMMIT/ROLLBACK e comandos `pgtest *`
   - PGTest adquire advisory lock baseado no Test-ID
   - PGTest executa query na conexão PostgreSQL real (usando Test-ID internamente)
   - PGTest libera advisory lock
...
public function handle($request, Closure $next)
{
    ...
    if ($testId) {
        // Modifica connection string para incluir application_name
        // application_name é parâmetro padrão PostgreSQL - todas as bibliotecas suportam
        $host = config('database.connections.pgsql.host');
        $port = config('database.connections.pgsql.port');
        $database = config('database.connections.pgsql.database');
        
        // Formato padrão PostgreSQL DSN
        $dsn = "pgsql:host={$host};port={$port};dbname={$database};application_name=pgtest_{$testId}";
...
**Ou via URI format (também padrão PostgreSQL):**
```php
$user = config('database.connections.pgsql.username');
$pass = config('database.connections.pgsql.password');
$dsn = "postgresql://{$user}:{$pass}@{$host}:{$port}/{$database}?application_name=pgtest_{$testId}";
```

**Como funciona:**
1. Cada requisição HTTP passa o Test-ID via header `X-Test-Run-Id`
2. O middleware modifica a connection string para incluir `application_name=pgtest_{$testId}`
3. `application_name` é parâmetro padrão do PostgreSQL (suportado por todas as bibliotecas)
4. O PGTest extrai o Test-ID do `application_name` no StartupMessage
5. O PGTest usa locks para acesso thread-safe ao pool de sessões
6. O PGTest roteia todas as queries para a transação correspondente ao Test-ID
7. Cada Test-ID tem sua própria transação isolada

**Exemplo de uso:**
```php
// Requisição 1: Test-ID = "test1"
// Connection string: pgsql:host=pgtest-host;port=5432;dbname=mydb;application_name=pgtest_test1
// Todas as queries vão para transação do test1

// Requisição 2: Test-ID = "test2"  
// Connection string: pgsql:host=pgtest-host;port=5432;dbname=mydb;application_name=pgtest_test2
// Todas as queries vão para transação do test2 (isolada do test1)

// Requisição 3: Test-ID = "test1" novamente
// Reutiliza a mesma transação do test1 (via pool com locks)
```
...
- [ ] Parse StartupMessage e extrair Test-ID de `application_name` (padrão PostgreSQL)
- [ ] Pool de conexões PostgreSQL por Test-ID com locks em memória (sync.RWMutex)
- [ ] Implementar GetOrCreateSession com locks para thread-safety do pool
- [ ] **Implementar Advisory Locks do PostgreSQL para serializar queries na mesma conexão**
- [ ] Adquirir `pg_advisory_lock` antes de executar query na conexão compartilhada
- [ ] Liberar `pg_advisory_unlock` após execução da query
- [ ] Proxy básico de queries com advisory locks
- [ ] Interceptar comandos especiais: `pgtest begin`, `pgtest rollback`, `pgtest status`, `pgtest list`, `pgtest cleanup`
- [ ] Testes básicos de conexão e concorrência (múltiplas requisições simultâneas)
