...
- Opções de implementação:
  - **Via parâmetro customizado:** `pgtest.test_id=abc123` na URI de conexão
  - **Via application_name:** `application_name=pgtest_abc123` (extrai `abc123`)
  - **Via variável de sessão:** `SET pgtest.test_id = 'abc123'` após conexão
- PGTest extrai o Test-ID do parâmetro de conexão (não mais do database name)
- O database name permanece o mesmo do PostgreSQL real (ex: `mydb`)
...
### 5. Interceptação de BEGIN/COMMIT/ROLLBACK
...
### 6. API de Controle HTTP

**Endpoints:**

```
POST /rollback?test_id=abc123
  - Executa ROLLBACK na transação do Test-ID
  - Fecha conexão
  - Remove do pool
...
2. **PGTest extrai Test-ID:**
   ```go
   testID := extractTestID(startupMsg.Parameters["database"])
   // test_abc123_mydb → abc123
   ```
...
'pgsql' => [
    'driver' => 'pgsql',
    'host' => env('DB_PROXY_HOST', '127.0.0.1'), // PGTest proxy
    'port' => env('DB_PROXY_PORT', '5432'),
    'database' => env('DB_DATABASE', 'laravel'),
    // ...
],

// .env
DB_PROXY_HOST=pgtest-service-host
DB_PROXY_PORT=5432
DB_DATABASE=mydb
```

### Middleware Laravel (para modificar database name)
...
public function handle($request, Closure $next)
{
    ...
    if ($testId) {
        // Modificar database name para incluir test_id
        Config::set('database.connections.pgsql.database', 
            "test_{$testId}_" . env('DB_DATABASE'));
    }
...
```php
// No final do teste E2E (Playwright)
await page.request.post('http://pgtest:8080/rollback?test_id=abc123');
```
...
2. **Laravel conecta:**
   - Database name: `test_abc123_mydb`
...
6. **Teste termina:**
   - Playwright: `POST /rollback?test_id=abc123`
...
- [ ] Parse StartupMessage e extrair Test-ID
