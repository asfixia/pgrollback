...
**Método: Via application_name (PostgreSQL nativo)**
- O Test-ID é passado via parâmetro `application_name` na conexão PostgreSQL
- Formato: `application_name=pgtest_abc123` → Test-ID: `abc123`
- **Não é extraído do database name** - o database name permanece o mesmo do PostgreSQL real
...
1. **Cliente (Laravel) conecta:**
   ...
   StartupMessage {
     ...
     parameters: {
       ...
       user: "myuser"
       application_name: "pgtest_abc123"   // Test-ID via application_name
     }
   }
   ```
...
public function handle($request, Closure $next)
{
    ...
    if ($testId) {
        // Forma mais simples: definir application_name via SQL
...
**Alternativa:** Modificar a connection string para incluir `application_name` diretamente:
```php
// Modificar a URI de conexão para incluir application_name
