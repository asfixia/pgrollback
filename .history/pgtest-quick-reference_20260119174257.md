...
2. **Cada requisição passa Test-ID via header `X-Test-Run-Id`**
3. Middleware configura `application_name=pgtest_{$testId}` na conexão
4. PGTest extrai Test-ID (`abc123`) e usa internamente para rotear conexões
5. PGTest conecta ao PostgreSQL real sem passar o Test-ID (usa `application_name` normal)
6. PGTest mantém pool interno: `Test-ID → conexão PostgreSQL`
