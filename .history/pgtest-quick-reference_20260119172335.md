...
1. Laravel conecta ao PGTest (como se fosse PostgreSQL)
2. PGTest extrai Test-ID do database name: `test_abc123_mydb` → `abc123`
3. PGTest mantém pool: `Test-ID → conexão PostgreSQL`
4. Múltiplas requisições com mesmo Test-ID → mesma conexão → mesma transação ✅
5. Rollback via HTTP: `POST /rollback?test_id=abc123`
