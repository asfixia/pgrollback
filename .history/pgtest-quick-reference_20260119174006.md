...
// Middleware: passar Test-ID para o PGTest via application_name
// O PGTest usa isso internamente, mas NÃO passa para o PostgreSQL real
DB::statement("SET application_name = 'pgtest_{$testId}'");

// Ou modificar a connection string para incluir:
// postgresql://user:pass@pgtest-host:5432/mydb?application_name=pgtest_abc123
```

**Importante:** O Test-ID é usado apenas pelo PGTest para roteamento interno. O PostgreSQL real não recebe essa informação.
