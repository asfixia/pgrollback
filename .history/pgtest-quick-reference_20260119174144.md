...
2. Laravel passa Test-ID via `application_name=pgtest_abc123` para o PGTest
3. PGTest extrai Test-ID (`abc123`) e usa internamente para rotear conexões
4. PGTest conecta ao PostgreSQL real sem passar o Test-ID (usa `application_name` normal)
5. PGTest mantém pool interno: `Test-ID → conexão PostgreSQL`
6. Múltiplas requisições com mesmo Test-ID → mesma conexão → mesma transação ✅
7. Comandos especiais para gerenciar transações: `pgtest begin abc123` e `pgtest rollback abc123`
...
// Middleware: passar Test-ID na conexão (forma mais simples)
public function handle($request, Closure $next)
{
    $testId = $request->header('X-Test-Run-Id');
    
    if ($testId) {
        // Define application_name antes de qualquer query
        // O PGTest extrai o Test-ID e roteia para a conexão correta
...
**Forma ainda mais simples - via connection string:**
```php
// Modificar a connection string diretamente
$dsn = "pgsql:host=pgtest-host;port=5432;dbname=mydb;application_name=pgtest_{$testId}";
```

**Importante:** O Test-ID é usado apenas pelo PGTest para roteamento interno. Cada Test-ID tem sua própria transação isolada.
