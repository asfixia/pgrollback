...
5. PGTest usa **Advisory Locks do PostgreSQL** para sincronizar requisições à mesma conexão
6. PGTest conecta ao PostgreSQL real sem passar o Test-ID (usa `application_name` normal)
7. PGTest mantém pool interno: `Test-ID → conexão PostgreSQL`
8. **Múltiplas requisições com mesmo Test-ID → mesma conexão → mesma transação ✅**
9. **Advisory locks garantem que queries não executem simultaneamente na mesma transação ✅**
10. **Cada Test-ID diferente → transação isolada ✅**
11. Comandos especiais para gerenciar transações: `pgtest begin abc123` e `pgtest rollback abc123`
...
- **Advisory locks do PostgreSQL** garantem que requisições simultâneas à mesma conexão sejam serializadas
- PGTest usa locks em memória (sync.RWMutex) para acesso thread-safe ao pool
...
- [ ] Locks em memória (sync.RWMutex) para acesso seguro ao pool em ambiente concorrente
- [ ] **Advisory locks do PostgreSQL** para serializar queries na mesma conexão/transação
- [ ] Adquirir advisory lock antes de executar query na conexão compartilhada
- [ ] Liberar advisory lock após execução da query
