...
var (
	...
	trayDblClickMu        sync.Mutex
	onTrayLeftDoubleClick func()
)
...
// SetOnTrayLeftDoubleClick registers a callback for a left double-click on the
// tray icon (Windows). On macOS and Linux it is currently unused. The callback
// runs on a new goroutine. Safe to call from any goroutine.
func SetOnTrayLeftDoubleClick(fn func()) {
	trayDblClickMu.Lock()
	onTrayLeftDoubleClick = fn
	trayDblClickMu.Unlock()
}

func notifyTrayLeftDoubleClick() {
	trayDblClickMu.Lock()
	fn := onTrayLeftDoubleClick
	trayDblClickMu.Unlock()
	if fn != nil {
		go fn()
	}
